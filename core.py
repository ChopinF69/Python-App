...
from utils.utils import my_sort_1 , my_sort_2 , my_sort_3
def ISTVAN():
    ...
    while True:
        ...
        if optiune == 1:
           ...
           # for client in listaClienti:
                #print(str(client))
            Consola.UI_afiseaza_clienti_recursiv(listaClienti , len(listaClienti) , 0)
            ...
            #for film in listaFilme:
                #print(str(film))
            Consola.UI_afiseaza_filme_recursiv(listaFilme , len(listaFilme) , 0)
           ...
           # else:
            ...
            for inchiriere in listaInchireri:
                ...
                # film = repoFilm.search_film_by_id(idFilm)
                film = serviceFilm.service_search_film_by_id(idFilm)
                new_list.append((client , film))
            #Consola.UI_afiseaza_inchirieri_recursiv(listaInchireri , len(listaInchireri) , 0)
            Consola.UI_afiseaza_inchirieri_recursiv(new_list , len(new_list) , 0)
        elif optiune == 16:
            ...
            #vreau sa implementez cu functia mea de sortare my_sort_1
            #sorted_result = dict(my_sort_1(result_dict.items() , key=lambda item : item[1] , reverse=True))
            sorted_list = dict(my_sort_1(result_dict.items(), key=lambda x: x[1], reverse=True))
            for nume_client, cnt in sorted_list.items():
            ...
            #sorted_result = dict(sorted(result_dict.items() , key=lambda item : item[1] , reverse=True))
            sorted_result = dict(my_sort_2(result_dict.items() , key=lambda item : item[1] , reverse=True))
            ...
            #o sa sortam dupa 2 parametrii alesi de mine
            #dupa numarul de filme si dupa nume - sortati alfabetic
            listaClienti = serviceClient.service_get_all_clients()
            ...
            noua_lista_sortata = []
            for nume_client, cnt in sorted_list.items():
                noua_lista_sortata.append((nume_client , cnt))

            #acuma vreau sa sortez dupa nume
            #noua_lista_sortata = my_sort_3(noua_lista_sortata , key1=lambda x : x[1] , key2=lambda x : x[0] , reverse=True)
            for i in range(0, len(noua_lista_sortata)):
                for j in range(i + 1, len(noua_lista_sortata)):
                    nume1 = noua_lista_sortata[i][0]
                    nume2 = noua_lista_sortata[j][0]
                    cnt1 = noua_lista_sortata[i][1]
                    cnt2 = noua_lista_sortata[j][1]

                    #print(nume1 , nume2)
                    if (cnt1 > cnt2 and nume1 == nume2):
                        noua_lista_sortata[i], noua_lista_sortata[j] = noua_lista_sortata[j], noua_lista_sortata[i]
            #for nume_client, cnt in sorted_list.items():
                #print(f"Clientul {nume_client} a efectuat {cnt} inchirieri.")

            for i in range(0 , len(noua_lista_sortata)):
                print(f"Clientul {noua_lista_sortata[i][0]} are  {noua_lista_sortata[i][1]} filme inchiriate")
