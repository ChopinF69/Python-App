...
class Console:
    ...
    def UI_afiseaza_meniu():
        ...
        print("24 . Obtine top utilizatori cu cele mai multe filme inchiriate in ordine alfabetica")
    @staticmethod
    ...
    @staticmethod
    def UI_afiseaza_inchirieri_recursiv(lista , length , index):
        if index == length:
            return
        nume = lista[index][0].get_nume()
        titlu = lista[index][1].get_titlu()
        print(f"Clientul cu numele {nume} a inchiriat filmul cu titlul {titlu}")
        Console.UI_afiseaza_inchirieri_recursiv(lista , length , index + 1)
    @staticmethod
    def UI_afiseaza_clienti_recursiv(lista , length , index):
        if index == length:
            return
        nume = lista[index].get_nume()
        cnp = lista[index].get_cnp()
        print(f"Clientul cu numele {nume} are cnp-ul {cnp}")
        Console.UI_afiseaza_clienti_recursiv(lista , length , index + 1)
    @staticmethod
    def UI_afiseaza_filme_recursiv(lista , length , index):
        if index == length:
            return
        titlu = lista[index].get_titlu()
        descriere = lista[index].get_descriere()
        gen = lista[index].get_gen()
        print(f"Filmul cu titlul {titlu} are descrierea {descriere} si genul {gen}")
        Console.UI_afiseaza_filme_recursiv(lista , length , index + 1)
